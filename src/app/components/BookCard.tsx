...
export default function BookCard({ book }: { book: any }) {
	return (
		<div className="mx-auto max-w-sm bg-white rounded-lg shadow-md p-4 m-2 border-2 border-white hover:border-blue-200">
			<div className="bg-blue-200 h-32 w-full rounded-md mb-2"></div>
			<h2 className="text-lg font-semibold text-gray-800" dir="auto">
				{book.title}
			</h2>
			<Link href={'/authors/' + book.author}>
				<p className="text-sm text-gray-600 hover:text-orange-600" dir="auto">
					{book.authors?.name}
				</p>
			</Link>
			...
			<div
				...
				role="figure"
			>
				<RateStar rating={book.average_rating} />
				<div className="flex" aria-hidden="true">
					<span className="pr-1">{book.average_rating} </span>
					{book.user_book && book.user_book[0]?.rating ? (
			...
			{book.user_book && book.user_book[0]?.date_added ? (
				<p className="text-sm text-gray-600">
					added on{' '}
					{new Date(book.user_book[0]?.date_added).toLocaleDateString('en-US', {
						dateStyle: 'long',
					})}
				</p>
			) : null}
